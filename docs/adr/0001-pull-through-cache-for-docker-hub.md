...
- **Refresh semantics**: `registry:2`'s `proxy.ttl` is set to a week. Moving tags (e.g. `node:25-alpine`) re-check upstream on that cadence; pinned tags (e.g. `node:25.1.0-alpine`) are effectively immutable and remain served from cache indefinitely.
